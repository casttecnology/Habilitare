	...
	if($numresp <> 0){
		while($perguntas = mysql_fetch_array($con2->resultado)){
			...
			//$categoria = $perguntas["cat_nome"];
				...
				{
				echo $questiontrue.' <img src="imagens/delete.ico" height="10px"><br>';
				}
		}
...
$con->selTab("","resultado_aluno,testes,categoria","distinct re_pergunta,re_resposta,te_pergunta,te_respcorreta, max(re_hora)","re_aluno = ".$user." and categoria.cat_curso = ".$cursoid." and categoria.cat_id = testes.te_curso and cat_id = te_curso and te_id = re_pergunta group by re_pergunta","re_data asc");
...
if($porcent >= 70){
	echo '<span style="font-size:16px;color:#FFF">Porcentagem de acertos : '.arredondar($porcent).' %</br></br>';
...
echo "Porcentagem de acertos : ".arredondar($porcent)." %</br></br>";
